...
class _LastOrderWidgetState extends State<LastOrderWidget> {
  ...
  Future<void> fetchLastRide() async {
    ...
    if (rides.isEmpty) {
      ...
      if (historyRes.succeeded) {
        var fromRides =
            DriverRideHistoryCall.completedRides(historyRes.jsonBody);
        if (fromRides.isEmpty) {
          fromRides =
              DriverRideHistoryCall.completedOrders(historyRes.jsonBody);
        }
        if (fromRides.isEmpty) {
    ...
    if (rides.isNotEmpty && mounted) {
      ...
      rides.sort((a, b) {
        final aVal =
            (a is Map ? a['completedAt'] ?? a['createdAt'] ?? a['date'] : null)
                    ?.toString() ??
                '';
        final bVal =
            (b is Map ? b['completedAt'] ?? b['createdAt'] ?? b['date'] : null)
                    ?.toString() ??
                '';
  ...
  Widget _buildRouteInfoCard() {
    String pickup = lastRide!['pickupAddress'] ??
        lastRide!['pickup_address'] ??
        lastRide!['from'] ??
        'Unknown Pickup Location';
    String drop = lastRide!['dropAddress'] ??
        lastRide!['drop_address'] ??
        lastRide!['to'] ??
